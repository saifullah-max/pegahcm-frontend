...
const AttendanceCard: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState('lastMonth'); // Default to lastMonth
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Map attendance by date for last month
                const attendanceByDate = new Map<string, any>();
                attendanceRecords.forEach((r: any) => {
                    const dateKey = new Date(r.date).toISOString().split('T')[0];
                    // Only keep last month records
                ...
                // Build daily attendance data for last month
                ...
                for (
                    ...
                    if (record) {
                        ...
                        const checkOut = checkOutHour !== null ? `${checkOutHour.toString().padStart(2, '0')}:00` : '--:--';
                        ...
                        dailyData.push({
                            ...
                            checkOut: '--:--',
                            actualHours: 0,
                            isAbsent: false, // Leave, not absent
                        });
                    } else if (d <= today) {
                        absent++;
                        dailyData.push({
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div className="flex space-x-2">
                    {/* Optional tabs if you want */}
                    {/* <button
            onClick={() => setActiveTab('lastMonth')}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${
              activeTab === 'lastMonth' ? 'bg-[#255199] text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 hover:bg-gray-300 dark:hover:bg-gray-600'
            }`}
          >
            Last Month
          </button> */}
            ...
            {/* Daily Bar Chart for last month */}
            <div className="pt-4 border-t dark:border-gray-700">
                <h2 className="text-xl font-medium text-gray-800 dark:text-gray-100 mb-4">Daily Attendance (Last Month)</h2>
...
const StatBox = ({ label, value, color }: { label: string; value: number; color: string }) => (
    <div className={`bg-${color}-100 dark:bg-${color}-800 bg-opacity-50 p-2 rounded-lg text-center w-full`}>
        <p className={`text-xs font-medium text-${color}-800 dark:text-${color}-100 truncate`}>{label}</p>
